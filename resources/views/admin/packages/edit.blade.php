            ...
            <form method="POST" action="{{ route("admin.packages.update", [$package->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('body') ? 'is-invalid' : '' }}" name="body"
                              id="body">{!! old('body', $package->body) !!}</textarea>
                    @if($errors->has('body'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number"
                           name="price" id="price" value="{{ old('price', $package->price) }}" step="1" required>
                    @if($errors->has('price'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('unit_price') ? 'is-invalid' : '' }}" type="text"
                           name="unit_price" id="unit_price" value="{{ old('unit_price', $package->unit_price) }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach(App\Models\Package::STATUS_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('status', $package->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
