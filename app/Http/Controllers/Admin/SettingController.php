...
class SettingController extends Controller
{
    ...
    public function store(StoreSettingRequest $request)
    {
        ...
        if ($request->input('featured_image', false)) {
            $setting->addMedia(storage_path('tmp/uploads/'.basename($request->input('featured_image'))))->toMediaCollection('featured_image');
        }
    ...
    public function update(UpdateSettingRequest $request, Setting $setting)
    {
        ...
        if ($request->input('featured_image', false)) {
            if (! $setting->featured_image || $request->input('featured_image') !== $setting->featured_image->file_name) {
                if ($setting->featured_image) {
                    $setting->featured_image->delete();
                }
                $setting->addMedia(storage_path('tmp/uploads/'.basename($request->input('featured_image'))))->toMediaCollection('featured_image');
            }
        } elseif ($setting->featured_image) {
            $setting->featured_image->delete();
        }
